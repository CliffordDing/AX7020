...
void bmp_load(char * bmp,u8 *frame,u32 stride)
{
	...
	res = f_open(&fil, bmp, FA_OPEN_EXISTING | FA_READ);  //加上要显示的图片文件名
	if(res != FR_OK)
	{
		return ;
	}
	...
	for(y = Yimage - 1;y >=0 ; y--)
	{
		f_read(&fil, &line_buf[0], Ximage * 3, &br);
		for(x = 0; x < Ximage; x++)
		{
			frame[x * 4 + iPixelAddr + 0] = line_buf[x * 3 + 0];
			frame[x * 4 + iPixelAddr + 1] = line_buf[x * 3 + 1];
			frame[x * 4 + iPixelAddr + 2] = line_buf[x * 3 + 2];
		}
		iPixelAddr += stride;
	}
	f_close(&fil);
...
int main(void)
{
	...
	bmp_load("1.bmp",dispCtrl.framePtr[dispCtrl.curFrame],1920 * 4);
