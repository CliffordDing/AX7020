...
*
*               (a)     \<Your Product Application>\clk_cfg.h
*
...
#include  <cpu_core.h>                                          /* CPU Core Library         (see Note #2)               */
...
typedef  enum   clk_err {
    CLK_ERR_NONE           = 0u,                                /* No err.                                              */

    CLK_ERR_NOT_NULL_PTR   = 10,
    CLK_ERR_NOT_INIT       = 11,
    CLK_ERR_ALLOC          = 20,
                                                                /* -------- CLOCK-OS LAYER ERROR CODE DEFINES --------- */
    CLK_OS_ERR_NONE        = 100u,

    CLK_OS_ERR_INIT_TASK   = 101u,
    CLK_OS_ERR_INIT_SIGNAL = 102u,
    CLK_OS_ERR_INIT_NAME   = 103u,

    CLK_OS_ERR_WAIT        = 120u,
    CLK_OS_ERR_SIGNAL      = 121u
} CLK_ERR;
...
CPU_BOOLEAN  Clk_SetTZ              (       CLK_TZ_SEC      tz_sec);
                                                                    ...
                                                                    /* Get clk TS using a CLK_DATE_TIME struct.         */
CPU_BOOLEAN  Clk_GetDateTime        (       CLK_DATE_TIME  *p_date_time);
                                                                    /* Set clk TS using a CLK_DATE_TIME struct.         */
...
CLK_DAY      Clk_GetDayOfYr         (       CLK_YR          yr,
                                            ...
                                            CLK_DAY         day);
                                                                    ...
                                                                    /* Conv a date/time struct to a str.                */
...
#if (CLK_CFG_NTP_EN == DEF_ENABLED)                                 /* --------------- NTP TS GET & SET --------------- */
                                                                    /* Get clk TS using NTP TS.                         */
CPU_BOOLEAN  Clk_GetTS_NTP          (       CLK_TS_SEC     *p_ts_ntp_sec);
                                                                    /* Set clk TS using NTP TS.                         */
...
#if (CLK_CFG_UNIX_EN == DEF_ENABLED)                                /* -------------- UNIX TS GET & SET --------------- */
                                                                    /* Get clk TS using Unix TS.                        */
CPU_BOOLEAN  Clk_GetTS_Unix         (       CLK_TS_SEC     *p_ts_unix_sec);
                                                                    /* Set clk TS using Unix TS.                        */
...
#error  "                                        [MUST be  > 0]                    "
        ...
        (CLK_CFG_SIGNAL_EN == DEF_DISABLED))
                                                                /* See 'clk.h  Note #3'.                                */
#error  "CLK_CFG_SIGNAL_EN must be set to DEF_ENABLED if CLK_CFG_EXT_EN is set to DEF_ENABLED"
#endif
...
#error  "LIB_VERSION       [SHOULD be >= V1.38]"
#endif

                                                                /* See 'clk.h  Note #1c'.                               */
#ifdef   COMMON_VERSION
#error  "Remove #ifdef  COMMON_VERSION once released."
#if     (COMMON_VERSION < 10000u)
#error  "COMMON_VERSION    [SHOULD be >= V1.00]"
#endif
#endif
