...
proc generate {ucos_handle} {
    ...
    if {$clk_enabled == "true"} {
        ...
        file copy -force "$ucos_src_base/uC-Clk/Cfg/Template/clk_cfg.c" "./src"
    }
    ...
    set heap_estimate [ucos_heap_estimate]
    set configured_heap [format "%u" [get_property CONFIG.LIB_MEM_CFG_HEAP_SIZE      $ucos_handle]]
    if {$configured_heap == 0} {
        set_define "./src/lib_cfg.h" "LIB_MEM_CFG_HEAP_SIZE" $heap_estimate
    } else {
        ucos_log_put "TRACE" "Overriding heap estimate with user defined value."
        set_define "./src/lib_cfg.h" "LIB_MEM_CFG_HEAP_SIZE" $configured_heap
    }
